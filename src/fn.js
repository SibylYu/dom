...
const reduce = emptyArray.reduce;
const push = emptyArray.push;
const sort = emptyArray.sort;
const splice = emptyArray.splice;
...
function concat() {
  ...
  for (i = 0; i < arguments.length; i++) {
    value = arguments[i];
    args[i] = $.isDom(value) ? value.toArray() : value;
  }
...
function on(...args) {
  ...
  // touch click 和 press 事件，与onclick事件只能二选一，使用 touch click，会抑制后续的onclick事件，避免事件穿透！
	// 用户一般不会调用e.preventDefault()来阻止穿透，因此在这里调用！
  function touchEnd(e) {
    ...
    if (x <= 5 && y <= 5) {
      e.preventDefault(); // 必须阻止后续的 onclick 事件，否则页面变动后，后续onclick 事件会触发在其他节点上，导致点击穿透错误！
...
function find(sel) {
  ...
  if (typeof sel == 'object')
    R = $(sel).filter(function () {
      const node = this;
      return emptyArray.some.call(self, function (parent) {
...
function att(name, v) {
  let n = [];
  try {
    n = this.dom?.querySelector(`[${name}=${v}]`);
  } catch (e) {}
  return $(n ? n : []);
}
...
function classes(cls) {
  ...
  try {
    ...
    R = this.dom?.querySelectorAll(rs.join(','));
    if (R && R.length > 0) R = Array.from(R);
  } catch (e) {}
